...
class Simulator:
    ...
    @staticmethod
    def fifty_one(miners_number=20, days=10):
        # Convert simulation days to seconds
        simulation_time = moment.get_seconds(days)
        try:
            clear_db()
        ...
        # Create the seed block
        seed_block = Block(None, 0, env.now, -1, 0, 1)
        hashes = numpy.random.dirichlet(numpy.ones(miners_number), size=1)
        hashrates = []
        for i in range(0, miners_number):
            hashrates.append(hashes[0,i])
        hashrates[miners_number-1] = 0.0
        hashrates[miners_number-1] = sum(hashrates) * (0.51/(1.0 - 0.51))
        hashrates = hashrates/sum(hashrates)
        # Create miners
        miners = []
        # This dict is used to store the connections between miners, so they are not created twice
        connections = dict()
        for i in range(0, miners_number - 1):
            miner = Miner(env, store, hashrates[i] * Miner.BLOCK_RATE, Miner.VERIFY_RATE, seed_block)
            miners.append(miner)
            connections[miner] = dict()
        # Create a bad miner
    ...
    @staticmethod
    def selfish(miners_number=20, days=10):
        # Convert simulation days to seconds
        simulation_time = moment.get_seconds(days)
        try:
            clear_db()
        ...
        for i in range(0, miners_number):
            hashrates.append(hashes[0,i])
        hashrates[miners_number-1] = 0.0
        hashrates[miners_number-1] = sum(hashrates) * (0.30/(1.0 - 0.30))
        hashrates = hashrates/sum(hashrates)
        # Create miners
        miners = []
        # This dict is used to store the connections between miners, so they are not created twice
